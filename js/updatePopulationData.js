...
/**
 * Fetch population data from Joshua Project API
 */
async function fetchPopulationData(name, country) {
    try {
        ...
        // Return just the population data
        return bestMatch.Population || '';
    } catch (error) {
...
/**
 * Process the CSV file and update missing population data
 */
async function updatePopulationData() {
    try {
        ...
        if (nameIndex === -1 || countryIndex === -1 || popIndex === -1) {
        ...
        // Count how many rows need population data
        const rowsNeedingPopulation = rowsAsArrays.slice(1).filter(row => 
            row.length > nameIndex && 
            row.length > countryIndex && 
            row[nameIndex] && 
            row[countryIndex] && 
            (row.length <= popIndex || !row[popIndex])
        );
        
        console.log(`Found ${rowsNeedingPopulation.length} rows with missing population data`);
        
        // Ask how many rows to process
        const processLimit = process.env.PROCESS_LIMIT ? 
            Math.min(parseInt(process.env.PROCESS_LIMIT), rowsNeedingPopulation.length) : 
            rowsNeedingPopulation.length;
        ...
        for (let i = 1; i < rowsAsArrays.length && processedCount < processLimit; i++) {
            ...
            const population = await fetchPopulationData(name, country);
            
            if (population) {
                // Ensure rowValues has enough elements
                while (rowValues.length <= popIndex) {
                    rowValues.push('');
                }
                
                // Update population value
                rowValues[popIndex] = population;
                console.log(`Updated population for ${name} in ${country}: ${population}`);
                updatedCount++;
            } else {
                console.log(`No population data found for ${name} in ${country}`);
            }
        ...
        console.log(`CSV file has been updated successfully! Updated population data for ${updatedCount} rows.`);
...
// Run the script
console.log('Starting Population data update process...');
updatePopulationData().then(() => {
    console.log('Population update process completed.');
}).catch(error => {
    console.error('Error in population update process:', error);
}); 